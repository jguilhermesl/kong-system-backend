...
const DEFAULT_SHEET_NAME = 'indications_kg_system';
const VALUE_INPUT_OPTION = 'RAW';
const DATA_STARTS_AT_LINE = 2;

type DaoType = Indication;
...
export class IndicationsDAO {
  ...
  private async getData() {
    ...
    try {
      ...
      const inventoryData = await inventoryDao.findMany({});

      const data =
        response.data.values?.map((item) => {

          const user = users.find((c) => c.id === item[2]) || null;
          const inventory = inventoryData.find((c) => c.id === item[1]) || null;
